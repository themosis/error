...
final class ExceptionHandlerHttpResponse {
	...
	public function render(Issue $issue): void {
        ...
        $content($this->view_path, [
            ...
            'preview' => function (Closure $preview_callback, Closure $line_callback) use ($issue) {
                return $this->render_preview($issue->preview(), $preview_callback, $line_callback);
            },
            'frames' => function (Closure $frames_callback, Closure $frame_callback, Closure $tag_callback, Closure $preview_callback, Closure $line_callback) {
                ...
                $frames = array_map(fn (Frame $frame) => $frame_callback(
    ...
    {
        $lines = array_map(static fn (string $line, int $number) => $line_callback(
            class_name: $file->is_current_line($number) ? 'current-line' : '',
            length: $file->row_number_length(),
            number: $number,
            line: $line,
        ), $file->get_lines(), array_keys($file->get_lines()));
